...
if( !function_exists( 'show_top_menu') )
{
	function show_top_menu() {
		...
		$count = count($pages);
		echo '<li><a href="' . get_home_url() . '/#intro">' . esc_attr('Home', 'Tharsis') . '</a>';
		for($i = 0; $i < $count; $i++)
		{
		...
		if(isset($tharsis['blog_page']) && $tharsis['blog_page'] != '')
			echo '<li><a href="' . get_permalink($tharsis['blog_page'][0]) . '">' . esc_attr('Blog', 'Tharsis') . '</a></li>';
		echo '<li><a href="' . get_home_url() . '/#contact-layout">' . esc_attr('Contact', 'Tharsis') . '</a></li>';
		echo '</ul>';
	}
}
...
if(! function_exists('vp_customization'))
{
	function vp_customization() {
		...
		if($tharsis['enable_colorization'] == 1)
		{
			...
			if($tharsis['bg_image'] != '')
			{ 
				echo "#intro, .footer { background-image: url('" . $tharsis['bg_image'] . "'); } \n";
			}
			...
			if($tharsis['bg_color'] != '') 
			{
				echo "#intro, .footer { background-image: none; background-color: " . $tharsis['bg_color'] . "; } \n";
			}
			echo '
