...
class ControllerModuleFeatured extends Controller {
	protected function index($setting) {
		...
		foreach ($products as $product_id) {
			...
			$specials_date = $this->model_catalog_LatestThemesCount->getProductDiscountDates($product_info['product_id']);
			$specials_purchased = $this->model_catalog_LatestThemesCount->getVouchersBought($product_info['product_id']);
			...
			if ($specials_date['date_end']) {				
				...
				$this->data['products'][] = array(
					...
					'thumb'   	 => $image,
					'name'    	 => utf8_substr(strip_tags(html_entity_decode($product_info['name'], ENT_QUOTES, 'UTF-8')), 0, 50) . ' ',
					'startdate'    	 => $product_info['date_added'],
					'date_end'  => $stringDate,
					'quantity' => $product_info['quantity'],
					'quantity_start' =>  $specials_purchased['quantity_purchased'],
					'description' => utf8_substr(strip_tags(html_entity_decode($product_info['description'], ENT_QUOTES, 'UTF-8')), 0, 40) . '..',
					'price'   	 => $price,
					'special' 	 => $special,
					'rating'     => $rating,
